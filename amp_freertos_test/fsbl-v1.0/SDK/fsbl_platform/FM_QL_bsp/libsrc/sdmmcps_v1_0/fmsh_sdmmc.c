...
static int sdmmc_waitStartCmdCleared(FSdmmcPs_T *pSdmmc)
{
    ...
    while (FMSH_ReadReg(pSdmmc->config.BaseAddress, SDMMC_CMD) & START_CMD)
    {
        timeout_cnt--;
        delay_1us();
        if (timeout_cnt == 0)
...
static int sdmmc_cmd0(FSdmmcPs_T *pSdmmc)
{
    ...
    pCmd->arg = 0;
    pCmd->res_type = CMD_RES_TYPE_NONE;
    pCmd->res_flag = 0;
...
static int sdmmc_cmd1(FSdmmcPs_T *pSdmmc)
{
    ...
    pCmd->res_type = CMD_RES_TYPE_R3;
    pCmd->res_flag = CMD_RES_FLAG_NOCRC;
    ret = pInstance->sendCmd(pSdmmc);
    if (ret < 0)
    {
...
*
*****************************************************************************/
static int sdmmc_sdCmd6(FSdmmcPs_T *pSdmmc, u32 Arg, u8 *pBuf)
{
    int ret;
    FSdmmcPs_SdCmd_T *pCmd;
    FSdmmcPs_Instance_T *pInstance;
    ...
    pCmd->arg = Arg;
    pCmd->res_type = CMD_RES_TYPE_R1;
    pCmd->res_flag = CMD_RES_FLAG_RDATA;
    pCmd->data = (char *)pBuf;
    pCmd->data_len = 512 / 8;
...
*
*****************************************************************************/
static int sdmmc_mmcCmd6(FSdmmcPs_T *pSdmmc, u32 Arg)
{
    int ret;
    FSdmmcPs_SdCmd_T *pCmd;
    FSdmmcPs_Instance_T *pInstance;
...
*
*****************************************************************************/
static int sdmmc_sdCmd8(FSdmmcPs_T *pSdmmc)
{
    int ret;
    FSdmmcPs_SdCmd_T *pCmd;
    FSdmmcPs_Instance_T *pInstance;
...
*
*****************************************************************************/
static int sdmmc_mmcCmd8(FSdmmcPs_T *pSdmmc, u8 *pBuf)
{
    int ret;
    FSdmmcPs_SdCmd_T *pCmd;
    FSdmmcPs_Instance_T *pInstance;
...
static int FSdmmcPs_readData(u32 baseAddr, FDmaPs_T *pDma, char *buf, int len, enum SDMMC_TransMode trans_mode) 
{
    ...
    if (trans_mode == sdmmc_trans_mode_dw_dma)
    {
        ...
        u32 buffer = (u32 )buf;
        unsigned int remain = len >> 2, trans_num = 0x400;
        ...
        while (remain)
        {
            ...
            if (remain > trans_num)
                ch_config.ctl_block_ts = trans_num;
            else
    ...
    else
    {
        ...
        if (((int)buf) & 0x03)
        {
            while(count < len)
            {
				sd_status =  FMSH_ReadReg(baseAddr, SDMMC_STATUS);
				if (!(sd_status & FIFO_EMPTY))  // not empty
				{
					fifo_cnt = SDMMC_STATUS_FIFO_COUNT(sd_status);
...
static int FSdmmcPs_writeData(u32 baseAddr, FDmaPs_T *pDma, char *buf, int len, enum SDMMC_TransMode trans_mode)
{
    ...
    else
    {
        ...
        if (((int)buf) & 0x03)
        {
            ...
            {
				sd_status =  FMSH_ReadReg(baseAddr, SDMMC_STATUS);
				if (!(sd_status & FIFO_FULL))  // not full
				{
					fifo_cnt = SDMMC_STATUS_FIFO_COUNT(sd_status);
					fifo_cnt = fifo_depth - fifo_cnt;
					for (i = 0; i < fifo_cnt; i++)
					{
                        temp = buf[count++];
                        temp += (buf[count++]<<8);
                ...
                if (timeout_cnt == 0)
                {
                    TRACE_OUT(DEBUG_OUT, "Write data timeout! \r\n");
                    return -FMSH_ETIME;
                }
            }
        }
        ...
        {
            timeout_cnt = 0x1000000;
			fifo_depth = FMSH_ReadReg(baseAddr, SDMMC_FIFOTH);
			fifo_depth = (((fifo_depth >> RX_WMARK_OFFSET) & RX_WMARK_MASK) + 1) * 2;
            ...
            while(count < len)
            {
                ...
                timeout_cnt--;
                if (timeout_cnt == 0)
                {   TRACE_OUT(DEBUG_OUT, "Write data timeout: [0x%04x], %d, %d, %d, %d\n\r",  sd_status, fifo_depth, fifo_cnt, count, len);
                    return -FMSH_ETIME;
                }
            }
        }
...
{
    pCmd->resp[0] = FMSH_ReadReg(baseAddr, SDMMC_RESP0);
}
...
{
    pCmd->resp[0] = FMSH_ReadReg(baseAddr, SDMMC_RESP0);
    pCmd->resp[1] = FMSH_ReadReg(baseAddr, SDMMC_RESP1);
    pCmd->resp[2] = FMSH_ReadReg(baseAddr, SDMMC_RESP2);
    pCmd->resp[3] = FMSH_ReadReg(baseAddr, SDMMC_RESP3);
}
...
void FSdmmcPs_instanceReset(FSdmmcPs_T *pSdmmc, FDmaPs_T *pDmac)
{
    ...
    pInstance->transMode = sdmmc_trans_mode_normal;
...
int FSdmmcPs_setClock(FSdmmcPs_T *pSdmmc, int rate)
{
    ...
    baseAddr = pSdmmc->config.BaseAddress;
    sd_clk = pSdmmc->config.InputClockHz;
    ...
    FMSH_WriteReg(baseAddr, SDMMC_CMD, temp);
...
u8 FSdmmcPs_getTransferMode(FSdmmcPs_T *pSdmmc)
{
    ...
    trans_mode = (FMSH_ReadReg(pSdmmc->config.BaseAddress, SDMMC_CMD) & TRANSFER_MODE) >> TRANSFER_MODE_OFFSET;
    return trans_mode;
}
...
int FSdmmcPs_sendCmd(FSdmmcPs_T *pSdmmc)
{
    ...
    if(pCmd->res_flag & (CMD_RES_FLAG_RDATA | CMD_RES_FLAG_WDATA))
    {
    ...
    FMSH_WriteReg(baseAddr, SDMMC_CMD, cmd_data);

    timeout_cnt = SDMMC_LOOP_TIMEOUT * 2;
    while (!(FSdmmcPs_getRawIntStat(pSdmmc) & RAW_INT_STATUS_CMDD))
    {
        timeout_cnt--;
        delay_1us();
        if (timeout_cnt == 0)
        {
            TRACE_OUT(DEBUG_OUT, "CMD done time out.\r\n");
            return -FMSH_ETIME;
        }
    }
    /* clear CMDD int */
    FSdmmcPs_setRawIntStat(pSdmmc, RAW_INT_STATUS_CMDD);
    
    if (pCmd->res_type != CMD_RES_TYPE_NONE) 
    {
        ...
        /* CRC? */
        else if (!(pCmd->res_flag & CMD_RES_FLAG_NOCRC)
        ...
        if (pCmd->res_flag & CMD_RES_FLAG_DATALINE) 
        {
            ret = FSdmmcPs_readData(baseAddr, pInstance->pDma, (char *)pCmd->resp, pCmd->data_len, pInstance->transMode);
        } 
        else 
        {
...
int FSdmmcPs_setCardBusWidth(FSdmmcPs_T *pSdmmc, unsigned int bus_width)
{
    ...
    else
    {
        if(pInstance->version >= MMC_VERSION_4)
        {
            while (retry)
            {
                ret = sdmmc_mmcCmd6(pSdmmc, MMC_4_BIT_BUS_ARG);
                if(ret < 0)
                {
                    if (ret == -FMSH_ENODEV)
                    {
...
int FSdmmcPs_getBusSpeed(FSdmmcPs_T *pSdmmc, u8 *pBuf)
{
    ...
    ret = sdmmc_sdCmd6(pSdmmc, arg, pBuf);
...
int FSdmmcPs_changeBusSpeed(FSdmmcPs_T *pSdmmc)
{
    ...
    if(pInstance->card_type == SD)
    {
        ...
        ret = sdmmc_sdCmd6(pSdmmc, arg, readBuff);
        if (ret < 0)
        {
            return ret;
        }
...
int FSdmmcPs_getSdSCR(FSdmmcPs_T *pSdmmc)
{
    ...
    TRACE_OUT(DEBUG_OUT, "scr is 0x%02x_%02x_%02x_%02x_%02x_%02x_%02x_%02x\r\n", pInstance->raw_scr[0], pInstance->raw_scr[1], pInstance->raw_scr[2],
              pInstance->raw_scr[3], pInstance->raw_scr[4], pInstance->raw_scr[5], pInstance->raw_scr[6], pInstance->raw_scr[7]);
    ...
    scr->sd_spec = pInstance->raw_scr[0] & 0xF;
    scr->bus_width = pInstance->raw_scr[1] & 0xF;
    scr->sd_spec3 = (pInstance->raw_scr[2] >> 7) & 0x1;
...
int FSdmmcPs_getMmcExtCsd(FSdmmcPs_T *pSdmmc)
{
    ...
    if (pInstance->version >= MMC_VERSION_4)
    {
        ...
        {
            memset(pInstance->ext_csd, 0, 512 * sizeof(u8));
            ret = sdmmc_mmcCmd8(pSdmmc, pInstance->ext_csd);
            if (ret < 0)
            {
                retry--;
                if(retry == 0)
...
int FSdmmcPs_sdEnumerate(FSdmmcPs_T *pSdmmc)
{
    ...
    memcpy(pInstance->raw_cid, pCmd->resp, 4 * sizeof(u32));
    ...
    pInstance->rca = pCmd->resp[0] >> 16;
    if (!(pCmd->resp[0] & STATE_IDENT))
    {
        return -FMSH_ENODEV;
    }
    ...
    memcpy(pInstance->raw_csd, pCmd->resp, 4 * sizeof(u32));
...
int FSdmmcPs_cardIdentify(FSdmmcPs_T *pSdmmc)
{
    ...
    else
    {
        ...
        if((pCmd->resp[0] & 0xFF) == (SD_CMD8_VOL_PATTERN & 0xFF))
        {
    ...
    if (pCmd->resp[0] & (R1_CC_ERR | R1_ERR))
    {
...
int FSdmmcPs_doTransfer(FSdmmcPs_T *pSdmmc, unsigned int address, unsigned int len,  int rw)
{
    ...
    else 
    {
        ...
        pCmd->res_type = CMD_RES_TYPE_R1;
        pCmd->res_flag = CMD_RES_FLAG_WDATA;
        pCmd->data = (char *)pInstance->priv;
        pCmd->data_len = len;
...
int FSdmmcPs_readFun(FSdmmcPs_T *pSdmmc, u32 address, char *pBuf, u32 len, enum SDMMC_TransMode mode)
{
    ...
    status = pInstance->doTransfer(pSdmmc, address, len, 1);
...
int FSdmmcPs_writeFun(FSdmmcPs_T *pSdmmc, u32 address, char *pBuf, u32 len, enum SDMMC_TransMode mode)
{
    ...
    pInstance->priv = pBuf;
    pInstance->transMode = mode;
    status = pInstance->doTransfer(pSdmmc, address, len, 0);
