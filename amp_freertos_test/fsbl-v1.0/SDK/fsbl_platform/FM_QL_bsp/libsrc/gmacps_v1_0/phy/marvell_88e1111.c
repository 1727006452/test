...
u8 mvl88e1111_get_realtime_link(FGmacPs_Instance_T * pGmac)
{
	u16 reg;
    u32 phy_timeout=0;

	/* wait realtime link up */
	phy_timeout=0;
    do{
		reg = mvl88e1111_reg_read(pGmac,PAGE0,PHY_88E1111_SST);
		phy_timeout++;
		if(phy_timeout>_88E1111_LINK_TIME_OUT){
            PHY_TRACE_OUT(FMSH_ENET_PHY_DEBUG, "PHY realtime link timeout \r\n");
			return ETHERNET_PHY_TIMEOUT;
		}
        delay_ms(1);
	}while((reg&PHY_88E1111_SST_LINK) == 0);
...
* @return: u32 states
*/
u32 mvl88e1111_pcs_lpbk(FGmacPs_Instance_T *pGmac, u8 enable, u8 speed)
{
    u32 reg;
    if(enable==1)
    {
        reg= mvl88e1111_reg_read(pGmac,0,PHY_88E1111_CTRL);
		RESET_BIT(reg,PHY_88E1111_CTRL_AN_EN);			/* disable AN*/
		SET_BIT(reg,PHY_88E1111_CTRL_CP_DPLX_MOD);		/* set full duplex */
		mvl88e1111_reg_write(pGmac,0,PHY_88E1111_CTRL,reg);
        /* set speed */
        reg= mvl88e1111_reg_read(pGmac,2,PHY_88E1111_CTRL);
        if(0==speed)/* 10M */
        {
            RESET_BIT(reg,PHY_88E1111_CTRL_SPEED_H);
            RESET_BIT(reg,PHY_88E1111_CTRL_SPEED_L);
        }else if(1==speed)/* 100M */
        {
            RESET_BIT(reg,PHY_88E1111_CTRL_SPEED_H);
            SET_BIT(reg,PHY_88E1111_CTRL_SPEED_L);
        }else if(2==speed)/* 1000M */
        {
            RESET_BIT(reg,PHY_88E1111_CTRL_SPEED_H);
            SET_BIT(reg,PHY_88E1111_CTRL_SPEED_L);
        }
        mvl88e1111_reg_write(pGmac,2,PHY_88E1111_CTRL,reg);
        /* reset */
        mvl88e1111_reset(pGmac);
        /* loopback */
        reg=mvl88e1111_reg_read(pGmac,0,PHY_88E1111_CTRL);
        SET_BIT(reg,PHY_88E1111_CTRL_LOOPBK);
        mvl88e1111_reg_write(pGmac,0,PHY_88E1111_CTRL,reg);
    }
    else
    {
        reg=mvl88e1116r_reg_read(pGmac,0,PHY_88E1111_CTRL);
        SET_BIT(reg,PHY_88E1111_CTRL_AN_EN);			/* enable AN*/
        RESET_BIT(reg,PHY_88E1111_CTRL_AN_EN);
        mvl88e1116r_reg_write(pGmac,0,PHY_88E1111_CTRL,reg);
        /* reset */
...
//{
//	FGmacPs_PhyConfig_T *phy_cfg_p = pGmac->phy_cfg;
//	u32 reg;
...
//	return ETHERNET_PHY_OK;
//}*/
//u32 mvl88e1111_timing_ctrl(FGmacPs_Instance_T *pGmac,u8 TxAddDelay,u8 RxAddDelay){
//    /* cfg */
//    u32 reg;
...
//* @return:
//*/
//u32 mvl88e_pcs_lpbk(FGmacPs_Instance_T *pGmac, u8 enable)
//{
//    u32 reg;
//    u32 phy_timeout;
//    if(enable==1)
...
//* @return:
//*/
//u32 mvl88e_line_lpbk(FGmacPs_Instance_T *pGmac, u8 enable)
//{
//    u32 reg;
//    if(enable==1)
//    {
