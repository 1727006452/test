                    ...
                    u8* pTxBuffer)
{
    ...
    * step 6.7.8 : Init Rx & Tx DES
    ...
    range_end = (((u32)pRxBuffer + pGmac->wRxListSize*2*GMAC_RBUFFER_UNIT_SIZE) & 0xffffffc0) + (1<<6);
    flush_dcache_range(range_start, range_end);
    range_start = (u32)pTxBuffer & 0xffffffc0; /* 64 byte aligned */
    range_end = (((u32)pTxBuffer + pGmac->wTxListSize*2*GMAC_TBUFFER_UNIT_SIZE) & 0xffffffc0) + (1<<6);
    flush_dcache_range(range_start, range_end);
#endif
    /*----------------------------
...
u8 FGmac_Ps_RcvPoll(FGmacPs_Instance_T *pGmac,u32 *pRcvSize)
{
    ...
    *pRcvSize = 0;
    ...
    invalidate_dcache_range(range_start, range_end);
        ...
        } else {  /* isFrame = 0; SOF has not been detected */
            pRxTd->RDES0.val |= GMAC_RDES0_OWN;            /* ownered by DMA */
            pGmac->wRxI = tmpIdx;
        }
...
u32 fmsh_clk_get_peripheral_pllout_rate(FGmacPs_Instance_T *pGmac, u32 pllid, u32 pllout_num)
{
	...
	case 1:   //armpll
		if (pllout_num < 3)
			div = FMSH_ReadReg(FPS_SLCR_BASEADDR,0x104+4) & PLLCLK_CTRL_DIV_MASK;
		else
			div = FMSH_ReadReg(FPS_SLCR_BASEADDR,0x104+4) & PLLCLK_CTRL_DIV_MASK;
		break;
	case 0:  //iopll
		div = FMSH_ReadReg(FPS_SLCR_BASEADDR,0x154+4) & PLLCLK_CTRL_DIV_MASK;
		break;
	default:
		return GMAC_RETURN_CODE_ERR;
	}
	...
	outrate = DIV_ROUND_CLOSEST(pllrate, div);
	printf("pllrate:%d,div:%d,outrate:%d\r\n", pllrate,div,outrate);
	return outrate;
}
...
u32 FGmac_Ps_Set_Gem_Rate(FGmacPs_Instance_T *pGmac,u32 GemClkRate)
{
	...
	div = DIV_ROUND_CLOSEST(pllout_rate, GemClkRate);
	printf("get clk pllout_rate %ld,div:%d,clk_ctrl:0x%x\r\n",pllout_rate,div,clk_ctrl);
	div = div & (CLK_GEM_TX_CTRL_DIV_MASK >> CLK_GEM_TX_CTRL_DIV_SHIFT);

	cur_rate = DIV_ROUND_CLOSEST(pllout_rate, div);
	if (cur_rate != GemClkRate) {
		printf("set clk %ld != %ld(required)\r\n",cur_rate, GemClkRate);
	}
	
	clk_ctrl &= ~CLK_GEM_TX_CTRL_DIV_MASK;
	clk_ctrl |= (div << CLK_GEM_TX_CTRL_DIV_SHIFT);
	printf("set clk cur_rate %ld,div:%d,clk_ctrl:0x%x\r\n",cur_rate,div,clk_ctrl);
