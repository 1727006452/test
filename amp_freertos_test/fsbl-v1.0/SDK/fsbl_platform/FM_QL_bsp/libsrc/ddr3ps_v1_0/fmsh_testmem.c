...
*           0xDEADBEEF is used.
* @param    Subtest is the test selected. See fmsh_testmem.h for possible
*	    values.
*
* @return
*
...
*
*****************************************************************************/
s32 Fmsh_TestMem32(u32 *Addr, u32 Words, u32 Pattern, u8 Subtest)
{
	u32 I;
	u32 j;
	u32 Val;
	...
	FMSH_ASSERT(Words != (u32)0);
	FMSH_ASSERT(Subtest <= (u8)FMSH_TESTMEM_MAXTEST);
	FMSH_ASSERT(Addr != NULL);

	/*
	 * variable initialization
	 */
	Val = FMSH_TESTMEM_INIT_VALUE;
	FirtVal = FMSH_TESTMEM_INIT_VALUE;


	if((Subtest == FMSH_TESTMEM_ALLMEMTESTS) || (Subtest == FMSH_TESTMEM_INCREMENT)) {
		/*
	...
	if((Subtest == FMSH_TESTMEM_ALLMEMTESTS) || (Subtest == FMSH_TESTMEM_WALKONES)) {
		/*
	...
	if((Subtest == FMSH_TESTMEM_ALLMEMTESTS) || (Subtest == FMSH_TESTMEM_WALKZEROS)) {
		/*
	...
	if((Subtest == FMSH_TESTMEM_ALLMEMTESTS) || (Subtest == FMSH_TESTMEM_INVERSEADDR)) {
	...
	if((Subtest == FMSH_TESTMEM_ALLMEMTESTS) || (Subtest == FMSH_TESTMEM_FIXEDPATTERN)) {
		/*
...
*
*****************************************************************************/
s32 Fmsh_TestMem16(u16 *Addr, u32 Words, u16 Pattern, u8 Subtest)
{
	u32 I;
	u32 j;
	u16 Val;
	...
	FMSH_ASSERT(Words != (u32)0);
	FMSH_ASSERT(Subtest <= FMSH_TESTMEM_MAXTEST);
	FMSH_ASSERT(Addr != NULL);
...
*
*****************************************************************************/
s32 Fmsh_TestMem8(u8 *Addr, u32 Words, u8 Pattern, u8 Subtest)
{
	u32 I;
	u32 j;
	u8 Val;
